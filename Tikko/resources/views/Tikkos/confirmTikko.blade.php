...
@section('content')
    <div class="bankView">
        ...
        <form action="{{ route('tikkos.store') }}" method="post">
            ...
            <div>
                <label for="receiverSelection">{{__('Select receivers')}}</label>
            ...
            <div>
                ...
                <button type="button" onclick="populateUnSelectedList()"
                        class="btn-primary">{{ __('Delete from receive list') }}</button>
            </div>
            <button type="submit" class="btn btn-success float-right" name="submit"
                    value={{$receiverCategory}}>{{ __('Send Tikko') }}</button>
        </form>
